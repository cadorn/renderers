...
exports.onLoad = template.onLoad = function(pack, tags){with(tags) {
    ...
    return {
        ...
        tag:
            DIV({"class": pack.__KEY__+"structures-trace"},
        ...
        shortTag:
            SPAN({"class": pack.__KEY__+"structures-trace"}, TAG("$node|getCaptionTag", {"node": "$node|getCaptionNode"})),
        ...
        onFileClick: function(event) {
            ...
            if(args["file"] && args["line"]) {
                this.dispatchEvent('inspectFile', [event, {
                    "args": args
                }]);
            }
        },
        ...
        getCaptionTag: function(node) {
            var rep = this.getRepForNode(this.getCaptionNode(node), true);
            ...
            try {
                ...
                for (var i = 0; i < frame.args.length; i++) {
                    items.push({
                        ...
                        "tag": this.getRepForNode(frame.args[i], true).shortTag,
